...
<div class="card shadow-xl bg-base-100">
    <div class="card-body">
        ...
        <x-form :action="route('users.update', $user->id)" method="POST">
            @csrf
            @method('PUT')

            <div class="form-control">
                <x-label for="name" value="Name" />
                <x-input id="name" name="name" type="text" class="input input-bordered w-full" :value="old('name', $user->name)" required autofocus />
                <x-error field="name" class="text-error text-sm mt-1" />
            </div>

            <div class="form-control mt-4">
                <x-label for="email" value="Email" />
                <x-input id="email" name="email" type="email" class="input input-bordered w-full" :value="old('email', $user->email)" required />
                <x-error field="email" class="text-error text-sm mt-1" />
            </div>

            <div class="form-control mt-4">
                <x-label for="password" value="New Password (Optional)" />
                <x-input id="password" name="password" type="password" class="input input-bordered w-full" />
                <x-error field="password" class="text-error text-sm mt-1" />
            </div>

            <div class="form-control mt-4">
                <x-label for="password_confirmation" value="Confirm New Password" />
                <x-input id="password_confirmation" name="password_confirmation" type="password" class="input input-bordered w-full" />
                <x-error field="password_confirmation" class="text-error text-sm mt-1" />
            </div>

            {{-- Optional: Role assignment on edit form --}}
            {{-- @if(isset($roles))
            <div class="form-control mt-4">
            ...
            <div class="card-actions justify-end mt-6">
                ...
                <x-form-button type="submit" class="btn btn-warning">Update User</x-form-button>
            </div>
        </x-form>
    </div>
</div>
